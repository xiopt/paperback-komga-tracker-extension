import { SearchRequest, PagedResults, SourceStateManager, RequestManager, Response } from '@paperback/types';
...
export async function searchRequest(
    ...
    console.log(`searchRequest succeeded with data: ${data}`);
...
export async function getOptions(stateManager: SourceStateManager): Promise<{
    ...
    const orderResultsAlphabetically =
        ((await stateManager.retrieve('orderResultsAlphabetically')) as boolean) ?? DEFAULT_SORT_RESULTS_ALPHABETICALLY;
    const showContinueReading =
        ((await stateManager.retrieve('showContinueReading')) as boolean) ?? DEFAULT_SHOW_CONTINUE_READING;
...
export async function retrieveStateData(stateManager: SourceStateManager) {
    ...
    const serverUsername =
        ((await stateManager.keychain.retrieve('serverUsername')) as string) ?? DEFAULT_KOMGA_USERNAME;
    const serverPassword =
        ((await stateManager.keychain.retrieve('serverPassword')) as string) ?? DEFAULT_KOMGA_PASSWORD;
    const showOnDeck = ((await stateManager.retrieve('showOnDeck')) as boolean) ?? DEFAULT_SHOW_ON_DECK;
    const orderResultsAlphabetically =
        ((await stateManager.retrieve('orderResultsAlphabetically')) as boolean) ?? DEFAULT_SORT_RESULTS_ALPHABETICALLY;
    const showContinueReading =
